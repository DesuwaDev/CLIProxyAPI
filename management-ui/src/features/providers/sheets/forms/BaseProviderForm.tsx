import { CredentialControls } from '@/features/nativeManagement/CredentialControls';
...
/** Module-level constants keep picker array references stable between renders. */
...
export function BaseProviderForm({
  ...
  /** Merge discovered and configured models; an empty catalog is normal, so the picker must also support plain rule editing. */
  ...
  return (
    <form id={formId} className={styles.form} onSubmit={handleSubmit} noValidate>
      ...
      {!descriptor.supportsApiKeyEntries && (
        <CredentialControls
          target={fallbackAuthIndex}
          disabled={mutating || !!form.apiKey?.trim()}
        />
      )}
      {/* Advanced sections */}
        ...
        <Collapsible label={t('providersPage.form.excludedSection')}>
          <div className={styles.field}>
            <ExcludedModelsPicker
              ...
              disabled={mutating}
              // '*' denotes a disabled provider, owned exclusively by the Disabled switch below.
              // The picker filters '*' in both directions and explains why manual entry is blocked.
